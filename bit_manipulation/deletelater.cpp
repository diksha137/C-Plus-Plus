...
std::uint64_t rightmostsetbit(std :: int64_t n) { // int64_t is preferred over int so that
    ...
    return static_cast<uint64_t>(log2(n & -n) + 1);
...
static void test() {
    // n = 4 return 3
    assert(bit_manipulation::rightmost_set_bit::rightmostsetbit(4) == 3);
    // n = 6 return 2
    assert(bit_manipulation::rightmost_set_bit::rightmostsetbit(6) == 2);
    // n = 12 return 3
    assert(bit_manipulation::rightmost_set_bit::rightmostsetbit(12) == 3);
    // n = 9 return 1
    assert(bit_manipulation::rightmost_set_bit::rightmostsetbit(9) == 1);
    // n = 15 return 1
    assert(bit_manipulation::rightmost_set_bit::rightmostsetbit(15) == 1);
    // n = 25 return 1
    assert(bit_manipulation::rightmost_set_bit::rightmostsetbit(25) == 1);
    // n = 96 return 6
    assert(bit_manipulation::rightmost_set_bit::rightmostsetbit(96) == 6);
    // n = 64 return 7
    assert(bit_manipulation::rightmost_set_bit::rightmostsetbit(64) == 7);
